...
.friendManager.getFriends().clear();
            ...
            FriendCommand.sendMessage(commands[0] + (uop
.friendManager.isFriend(commands[0]) ? " is friended." : " isn't friended."));
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
