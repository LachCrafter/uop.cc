...
public class ClickGui
    ...
    public ClickGui() {
        super("Client", "Opens the ClickGui", Module.Category.CLIENT, true, false, false);
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                uop
.commandManager.setPrefix(this.prefix.getPlannedValue());
                Command.sendMessage("Prefix set to " + Formatting.DARK_GRAY + uop
.commandManager.getPrefix());
            }
            uop
.colorManager.setColor(this.red.getPlannedValue(), this.green.getPlannedValue(), this.blue.getPlannedValue(), this.hoverAlpha.getPlannedValue());
        }
    }
    ...
    public void onLoad() {
        uop
.colorManager.setColor(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.hoverAlpha.getValue());
        uop
.commandManager.setPrefix(this.prefix.getValue());
    }
